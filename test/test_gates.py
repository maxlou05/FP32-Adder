...
async def test_project(dut):
    ...
    for i in range(4):
        ...
        # For some reason reading after awawiting ReadWrite() no longer works in the integration test,
        # and we have to wait an extra period, even though the waveform is exactly the same
        await RisingEdge(dut.clk)
        await ReadWrite()
